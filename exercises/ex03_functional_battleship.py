...
def main(grid: int, secret_row: int, secret_column: int) -> None:
    ...
    while turn <= 5 and has_won == False:
        print(f'=== Turn {turn}/5 ===')

        user_row = input_guess(grid, 'column') 
        user_column = input_guess(grid, 'row')
        correct_or_not = correct_guess(user_row, user_column, secret_row, secret_column)
        print_grid(grid, user_row, user_column, correct_or_not)
